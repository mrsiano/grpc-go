...
Clients may use
[metadata.MD](https://godoc.org/github.com/mrsiano/grpc-go/metadata#MD)
to store tokens and other authentication-related data. To gain access to the
`metadata.MD` object, a server may use
[metadata.FromIncomingContext](https://godoc.org/github.com/mrsiano/grpc-go/metadata#FromIncomingContext).
...
to lowercase. See [here](https://godoc.org/github.com/mrsiano/grpc-go/metadata#New).

It is possible to configure token validation for all RPCs using an interceptor.
A server may configure either a
[grpc.UnaryInterceptor](https://godoc.org/github.com/mrsiano/grpc-go#UnaryInterceptor)
or a
[grpc.StreamInterceptor](https://godoc.org/github.com/mrsiano/grpc-go#StreamInterceptor).

## Adding a token to all outgoing client RPCs

To send an OAuth2 token with each RPC, a client may configure the
`grpc.DialOption`
[grpc.WithPerRPCCredentials](https://godoc.org/github.com/mrsiano/grpc-go#WithPerRPCCredentials).
Alternatively, a client may also use the `grpc.CallOption`
[grpc.PerRPCCredentials](https://godoc.org/github.com/mrsiano/grpc-go#PerRPCCredentials)
on each invocation of an RPC.

To create a `credentials.PerRPCCredentials`, use
[oauth.TokenSource](https://godoc.org/github.com/mrsiano/grpc-go/credentials/oauth#TokenSource).
